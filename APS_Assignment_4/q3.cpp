...
//  const char* pat=pat1.head->str;
    ...
    for(int i=0;i<N;) { 
        ...
        if (j == M) { 
        	if(count==0){
        		int index=i-j;
            cout<<"Found pattern at index: "<<index; 
            j = presuf[j - 1];
            count++;
            }
            else{
...
int main() {  
	stringBuilder str1 =stringBuilder("bcbcbc");
	stringBuilder str2 =stringBuilder("bcbc");
	//stringBuilder str3 =append(str1,str2);
	//stringBuilder str4 =append(str1,str3);
//	print(str3);
	//cout<<str3.head->str;
	//char* a="nikn"; char* b="manikn";
